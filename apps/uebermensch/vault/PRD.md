...
1. **Vault is portable and Obsidian-mountable.** `$UBER_VAULT_DIR` is a git-versioned Obsidian vault outside the gctrl repo, readable without Uebermensch running. Every file is CommonMark markdown with YAML frontmatter; wikilinks use plain `[[slug]]` form (typed prefixes like `[[thesis:slug]]` are forbidden — they break Obsidian's resolver). The app MUST NOT write to the authored tier (`directives/`) without explicit user confirmation. The single exception is the prompts inbox at `directives/prompts/<slug>.md`, where the prompts processor (`gctrl uber prompts process`) is allowed to (a) update frontmatter in-place (`status`/`output`/`processed_at`/…) and (b) move the file to `directives/prompts/archived/<slug>.md` after a successful `kind: thought` run. The body is never touched. Authored theses, topics, sources, etc. remain user-only — even thesis _addendums_ proposed by the thought pipeline are surfaced as suggestions in the report for the user to paste, never auto-applied.
...
|--------|---------|------------------|--------|
| `driver-llm` | Unified LLM interface — **local-first**: default provider is LM Studio (`http://127.0.0.1:1234/v1/chat/completions`, default model `google/gemma-4-31b`). Cloudflare AI Gateway is opt-in via `GCTRL_LLM_PROVIDER=cloudflare`; Anthropic models still reachable via `/api/llm/messages`. Every call = Session + spans | `LlmPort` (new) | Planned (blocks M0) |
