...
| driver-llm: local-first adapter | Real client behind `LlmPort`. **Default**: LM Studio at `http://127.0.0.1:1234/v1/chat/completions`, default model `google/gemma-3-27b-it`. **Opt-in**: Cloudflare AI Gateway via `GCTRL_LLM_PROVIDER=cloudflare` (kernel holds `CF_API_TOKEN` + `CLOUDFLARE_AI_GATEWAY_ID`). Anthropic-shape models reachable via `/api/llm/messages` | P0 | M0 driver-llm stub | TBD |
