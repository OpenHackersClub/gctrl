...
gctrl inbox count                    # → "5 pending (2 critical, 1 high)"
gctrl inbox count --urgency high     # → "3 pending (high+)"
```
...
gctrl inbox list
gctrl inbox list --urgency high --kind permission_request
gctrl inbox list --project BACK --pending
gctrl inbox list --all  # include acted/dismissed/expired

# View single message with full context
gctrl inbox view msg_abc123

# View thread with all messages
gctrl inbox thread thr_def456
```
...
gctrl inbox approve msg_abc123
gctrl inbox approve msg_abc123 --reason "verified branch is safe to force-push"

# Deny with required reason
gctrl inbox deny msg_abc123 --reason "use rebase instead of force-push"

# Acknowledge informational message
gctrl inbox acknowledge msg_abc123

# Defer for later
gctrl inbox defer msg_abc123 --until 2h
gctrl inbox defer msg_abc123 --until "2026-04-03T18:00:00Z"

# Delegate to another user
gctrl inbox delegate msg_abc123 --to alice

# Reply to agent question
gctrl inbox reply msg_abc123 --body "Use the v2 API endpoint instead"

# Batch approve multiple messages
gctrl inbox batch-approve msg_abc123 msg_def456 msg_ghi789
gctrl inbox batch-approve msg_abc123 msg_def456 --reason "approved in batch review"
```
...
# List actions taken
gctrl inbox actions
gctrl inbox actions --actor alice --since 7d
gctrl inbox actions --thread thr_def456

# Inbox statistics
gctrl inbox stats
gctrl inbox stats --since 30d
```
...
gctrl inbox subscribe --filter project=BACK

# Subscribe to all high+ urgency messages
gctrl inbox subscribe --filter urgency_gte=high

# Unsubscribe
gctrl inbox unsubscribe sub_xyz789
```
