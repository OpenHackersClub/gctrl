...
flowchart TB
  ...
  subgraph App["APPLICATION: gctrl-board"]
...
> **Note:** This is an **application-level** interface, not a kernel interface trait. It lives in the gctrl-board package, not in `gctrl-core`. External app drivers that implement it are connecting to the application, not the kernel.
...
- [issue-lifecycle.md](../../gctrl/workflows/issue-lifecycle.md) — Kanban status definitions and transition rules
- [gctrl-board.md](gctrl-board.md) — Application-level view of the board (agent integration, kernel primitives used)
