...
A lightweight roadmap template for gctl-board projects. Captures milestones, task breakdowns, acceptance criteria, and open questions — separate from the PRD so the two can evolve at different cadences.
...
1. Copy this template into your project's directory (e.g., `specs/gctl/ROADMAP.md` or `apps/gctl-board/ROADMAP.md`).
2. Fill in milestones from the PRD's high-level capabilities.
3. Each milestone produces gctl-board issues: one row per task, linked to an issue ID.
...
Each row in a milestone table maps to a gctl-board issue:

```sh
# Create issues from milestone tasks
gctl board create --project PROJ --title "Task 1" --priority p0 --label m1
gctl board create --project PROJ --title "Task 2" --priority p1 --label m1

# Set dependencies between tasks
gctl board block PROJ-2 --by PROJ-1
```
...
3. Fill in Issue IDs as tasks are created in gctl-board — `TBD` rows are planning debt.
