...
A lightweight PRD template for gctl-board projects. Captures the **why** and **what** — business motivation, goals, use cases, and a roadmap that translates directly into issues.
...
1. Copy this template into your project's directory (e.g., `specs/gctl/PRD.md` or `apps/gctl-board/PRD.md`).
2. Fill in each section. Delete sections that don't apply.
3. The Roadmap section should produce issues: each milestone/feature maps to one or more gctl-board issues.
...
Each milestone maps to a set of issues. Use this to generate gctl-board issues.
...
The Roadmap section is designed to feed directly into `gctl board`:

```sh
# Create issues from milestone features
gctl board create --project PROJ --title "Feature 1" --priority p0 --label m1
gctl board create --project PROJ --title "Feature 2" --priority p1 --label m1
