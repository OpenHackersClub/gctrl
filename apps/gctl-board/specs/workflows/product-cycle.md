...
3. **Backlog** — `gctl board list --status backlog` — what's available.
4. **Existing specs** — architecture and implementation docs relevant to the work.

### 2.2 Agent Drafts Plan

The agent produces a **plan document** (posted as a gctl-board comment or PR) containing:
...
gctl board list --status todo
# Agent posts plan as a comment on the cycle issue
gctl board comment CYCLE-3 "$(cat .tmp/cycle-plan.md)"
```
...
1. **Roadmap update** — the agent MUST update `ROADMAP.md` to reflect what shipped, what moved, and what's next. The diff is part of the delivery:
   ...
   git diff HEAD~1 -- specs/gctl/ROADMAP.md
   ```
...
gctl board list --status backlog     # agent reads backlog
gctl board comment CYCLE-3 --body-file .tmp/cycle-plan.md  # agent posts plan
...
gctl cycle summary                   # generate delivery report
