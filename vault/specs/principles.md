...
4. **In-loop improvement, not out-of-band.** Performance signal (eval scores, judge metrics, review feedback, run outcomes) MUST be able to flow back into the agent harness (prompts, skill selection, scope, retry policy) without a human prompt-editing ritual. Manual prompt edits are a fallback, not the design.
