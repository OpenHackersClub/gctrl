...
5. **Capability requirements, not implementations**: An app declares the capabilities it needs via *ports*; the kernel fulfills them through drivers. Apps MUST NOT bundle in-app re-implementations of capabilities the kernel already provides (e.g. an in-app Anthropic adapter when `driver-llm` already exists). See [app-decoupling.md](app-decoupling.md) for the full contract and the zero-duplication rule.
6. **Ejection is fork-and-rewire**: Running an app on a non-gctrl host means forking and editing its single Layer module so ports call host-supplied implementations. Business logic, ports, the vault, and the `gctrl-app.toml` manifest stay unchanged. The app MUST NOT ship runtime override mechanisms (no `--override` install flag, no `UBER_CUSTOM_LAYER`-style env loaders) or "local-direct" / "cloud-only" mode adapters that re-implement kernel drivers. See [app-install-protocol.md § Non-gctrl hosts](app-install-protocol.md#non-gctrl-hosts).
...
apps/utils/skills/     # shipped with repo (e.g. /dispatch, /review)
```
