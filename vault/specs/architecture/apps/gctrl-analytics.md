...
4. **M3 — Attribution** *(shipped, including kind-aware rollups)*:
   ...
   - **M3 follow-up** *(shipped)*: `/api/analytics`, `/api/analytics/cost`, `/api/analytics/latency`, `/api/analytics/spans` all accept `?kind=` and `?created_by=`; storage methods take `Option<&[CreatedBy]>` and join `spans → sessions` when filtering. The Overview/Usage tabs thread the global `kind` through their queries; the previous "· all kinds" annotation is gone because the rollup KPIs now match the active filter. `/api/analytics/scores` and `/api/analytics/daily` are intentionally not yet kind-filtered: scores are name-keyed (cross-target), daily aggregates are pre-rolled into a `(date, metric, dimension)` table. Promote either when an operator-visible split surfaces them in the UI.
   - *Accept (sessions)*: filtering `kind=external` on a workspace with only scheduler-spawned sessions returns zero rows; `kind=internal` returns every row. Totals of the two equal the unfiltered total. **Verified** by `kernel/crates/gctrl-otel/tests/created_by_filter.rs::internal_plus_external_equals_total`.
   - *Accept (rollups)*: `cost(internal) + cost(external) == cost(all)` for both `by_model` and `by_agent`, same for `/api/analytics` totals. **Verified** by `kernel/crates/gctrl-otel/tests/analytics_kind_filter.rs::analytics_cost_split_by_kind_sums_to_total` and `::analytics_overview_split_by_kind_sums_to_total`.
5. **M4 — Network sub-panel** *(first cut shipped — host/process-level grouping; per-session attribution deferred)*: kernel `proxy` Phase 2 landed in #79 (hudsucker MITM + `TrafficRecord` writes through the existing `DuckDbStore`). M4 layers on the analytics rollup routes the spec called for: `/api/net/stats?since=` now populates all `TrafficStats` fields (no longer a stub returning only `total_requests`); new `/api/net/domains?since=&top=N` returns per-host requests + bytes; new `/api/net/daily?days=N` buckets requests/bytes by calendar date. UI ships as a sub-panel inside the **Usage tab** (not its own tab) — top domains table, status-code distribution, recent requests table, with a 1h/24h/7d/all window toggle. Empty state nudges the operator to run `gctrld serve --proxy` + `gctrl net setup`. **Per-session attribution is explicitly deferred** to a follow-up: requires the proxy to tag traffic with `Session-Id` (env-injected header or PID→session mapping). Until then, requests show host/process-level grouping only — that's what the spec promises in Kernel Deps §2.
   - *Accept (kernel rollups)*: stats split by `?since=` returns the right window (`net_stats_since_filter_excludes_old_rows`), domains rank by request count (`net_domains_orders_by_request_count`), daily buckets are calendar-day aggregates (`net_daily_buckets_by_calendar_date`). **Verified** by `kernel/crates/gctrl-otel/tests/net_proxy_routes.rs`.
   - *Accept (per-session drill-through)*: deferred. The original spec's acceptance criterion ("a request made through the proxy during a known session appears in that session's detail pane under outbound requests") needs proxy-side `Session-Id` tagging; until that lands, the Sessions detail pane has no `outbound requests` block.
6. **M5 — Contributions** *(PRs + commits + time filter shipped)*: `GET /api/contributions?repo=owner/name&kind=...&since=...&limit=N` (trailer-inference flavour per Kernel Dependencies §4). The kernel route makes two `gh` calls: `gh pr list --json body,... [--search created:>=YYYY-MM-DD]` for PRs and `gh api repos/{repo}/commits?per_page=N[&since=...]` for commits. Each row's message body is scanned for a canonical `Session-Id: <uuid>` trailer (`kernel/crates/gctrl-otel/src/contributions.rs`), then joined back to local sessions via `store.get_session`. Results are merged and sorted by `created_at` desc. `?since=` accepts `YYYY-MM-DD` or relative shorthand `7d`/`30d`/`90d` (resolved server-side via `resolve_since`); invalid input is silently ignored. Rows without a trailer are kept as `unattributed` when `kind=all`; the `kind` filter drops them (the operator narrowed to a known population). UI tab on `apps/gctrl-board/web/src/pages/AnalyticsPage.tsx` (`ContributionsTab`) lets the operator pick a repo (defaults to `OpenHackersClub/gctrl`, persisted in `localStorage`) and a 7d/30d/90d/all time range, and renders type / ref (PR# or short SHA) / title / author / session / state with drill-through to the Session detail pane. **Not yet shipped**: closed-issues join, sparkline split by `created_by`. No `session.contribution_links` table — promote only if inference proves lossy on a real workspace.
   - *Accept (trailer parser)*: `parse_session_trailer` accepts canonical trailers (case-insensitive, leading whitespace tolerated, first-trailer-wins) and rejects malformed UUIDs and inline URLs. **Verified** by `kernel/crates/gctrl-otel/src/contributions.rs` unit tests (7 tests).
   - *Accept (row builder + filter)*: a PR with a trailer joins to its session and surfaces `created_by`/`session_agent`; a PR without a trailer is kept unattributed; a `kind` filter drops unattributed rows. **Verified** by `kernel/crates/gctrl-otel/src/receiver.rs::tests::contribution_row_*` (5 tests).
