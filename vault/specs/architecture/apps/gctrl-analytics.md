...
6. **M5 — Contributions** *(initial cut shipped — PRs only)*: `GET /api/contributions?repo=owner/name&kind=...&limit=N` (trailer-inference flavour per Kernel Dependencies §4). The kernel route shells out to `gh pr list --json body,number,title,...`, scans each PR body for a canonical `Session-Id: <uuid>` trailer (`kernel/crates/gctrl-otel/src/contributions.rs`), and joins back to local sessions via `store.get_session`. Rows without a trailer are kept as `unattributed` when `kind=all`; the `kind` filter drops them (the operator narrowed to a known population). UI tab on `apps/gctrl-board/web/src/pages/AnalyticsPage.tsx` (`ContributionsTab`) lets the operator pick a repo (defaults to `OpenHackersClub/gctrl`, persisted in `localStorage`) and renders type/PR#/title/author/session/state with drill-through to the Session detail pane. **Not yet shipped**: `gh search commits` pass, closed-issues join, sparkline split by `created_by`. No `session.contribution_links` table — promote only if inference proves lossy on a real workspace.
