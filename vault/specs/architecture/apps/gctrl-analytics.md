...
6. **M5 — Contributions** *(PRs + commits shipped)*: `GET /api/contributions?repo=owner/name&kind=...&limit=N` (trailer-inference flavour per Kernel Dependencies §4). The kernel route makes two `gh` calls: `gh pr list --json body,...` for PRs and `gh api repos/{repo}/commits?per_page=N` for commits. Each row's message body is scanned for a canonical `Session-Id: <uuid>` trailer (`kernel/crates/gctrl-otel/src/contributions.rs`), then joined back to local sessions via `store.get_session`. Results are merged and sorted by `created_at` desc. Rows without a trailer are kept as `unattributed` when `kind=all`; the `kind` filter drops them (the operator narrowed to a known population). UI tab on `apps/gctrl-board/web/src/pages/AnalyticsPage.tsx` (`ContributionsTab`) lets the operator pick a repo (defaults to `OpenHackersClub/gctrl`, persisted in `localStorage`) and renders type / ref (PR# or short SHA) / title / author / session / state with drill-through to the Session detail pane. **Not yet shipped**: closed-issues join, sparkline split by `created_by`. No `session.contribution_links` table — promote only if inference proves lossy on a real workspace.
