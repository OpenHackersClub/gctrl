...
>
> Instantiates the [PRD template](../../apps/gctl-board/specs/workflows/prd-template.md) for gctl itself.
...
gctl applies the Unix model to harness engineering:
...
3. **Usable out of the box by one person.** `cargo install gctl && gctl serve` — no config files, no cloud accounts, no Docker. A solo developer gets telemetry, storage, guardrails, and orchestration immediately.
...
5. **Adapt, don't replace.** Connect to tools you already use via drivers. Shipped applications (gctl-board, Observe & Eval) are defaults, not mandates.
