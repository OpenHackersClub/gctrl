...
│   └── gctl-orch/             # Orchestration state machine proofs
│       ├── Gctl/Orch/
...
| Lean 4 compilation, proof checking | Lake | `lean4/*` |
...
// lean4/gctl-orch/project.json
{
  ...
  "targets": {
    ...
    "export-transition-table": {
      "executor": "nx:run-commands",
      "options": {
        "command": "lake run export-transition-table",
        "cwd": "lean4/gctl-orch"
      },
      "dependsOn": ["build"],
      "inputs": ["lean"]
...
// crates/gctl-orch/project.json (partial)
{
  "targets": {
    "test": {
      "dependsOn": ["gctl-orch-lean:build"]
    }
  }
}
```
...
3. **Lean 4 projects live in `lean4/`.** Named `gctl-{name}`. Managed by Lake. Each project has a `lakefile.lean` and pinned `lean-toolchain`.
