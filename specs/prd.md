...
## Our Take: What Solves This Fundamentally

**The fundamental insight is that agent teams need an operating system, not a collection of point tools.**
...
gctl applies this model to agent teams:

| Problem | Unix Solution | gctl Solution |
|---------|--------------|---------------|
...
**The kernel is small and always present.** You get telemetry, storage, guardrails, and orchestration by running `gctl serve` — no config, no cloud, no Docker. Everything else is optional: applications, drivers, cloud sync, browser control. Complexity is opt-in.

**Agents are first-class users, not just tools.** They have identities (personas), capability grants, resource limits, and process trees — just like Unix users. This makes the system composable, auditable, and safe by default.
...
**Adapt, don't replace.** gctl connects to tools you already use (Linear, Notion, Obsidian, Phoenix) via drivers. It provides the kernel underneath, not a replacement for your workflow.

## Design Principles

1. **Usable out of the box by one person.** `cargo install gctl && gctl serve` — no config files, no cloud accounts, no Docker. A solo developer gets telemetry, storage, guardrails, and orchestration immediately.
2. **Small kernel, optional everything else.** The kernel has four core primitives. Network control, browser control, cloud sync, and the scheduler are extensions. All applications and drivers are optional.
3. **Adapt, don't replace.** Connect to tools you already use via drivers. Shipped applications (gctl-board, Observe & Eval) are defaults, not mandates.
4. **OS layer is stable; applications evolve fast.** The telemetry format, storage schema, and CLI change rarely. Applications ship, iterate, and break independently.
5. **Applications share primitives, not state.** Apps use the same storage but own their table namespaces. Cross-app data flows through kernel events, not direct table joins.
6. **Agents are first-class consumers.** Every feature is CLI/API-first and automatable. No browser-only UIs.
7. **Local-first, cloud-optional.** The kernel works fully offline. Cloud sync is opt-in.
...
| "Dispatch work to my agent and track progress" | Orchestrator + gctl-board |
...
| Need | What They Use |
|------|-------------|
| "Sync our Linear/GitHub issues to gctl orchestration" | Drivers (driver-linear, driver-github) |
...
**gctl-board** — Lightweight project management and kanban. Issues with status lifecycle, dependency graph, agent assignment, auto-transitions from kernel events. The first application built on gctl.

**Observe & Eval** — Langfuse-grade analytics, local-first. Cost/token analytics, latency percentiles, trace exploration, scoring (human + auto + model), prompt version management. Everything Langfuse shows in its dashboard, gctl can produce from local DuckDB.
...
|---------|-------------|
| `gctl net fetch` | Fetch a URL, convert to markdown |
| `gctl net crawl` | Crawl a site, extract readable content |
| `gctl net compact` | Compact crawled pages into LLM-ready context |
| `gctl context add/list/compact` | Manage agent context (docs, configs, snapshots) |
| `gctl browser goto/snapshot` | Browser automation |
| `gctl query` | Guardrailed data access for agents |

### External App Integration (Drivers)

gctl does not replace your tools — it connects to them:
...
Zero drivers = gctl works standalone. Add drivers as your workflow grows.
...
**Solution:** Real-time cost tracking per session, per agent, per model. Budget guardrails pause or halt sessions that exceed thresholds. Daily/weekly cost trends. Per-issue cost attribution (via gctl-board).
...
**Solution:** Context manager stores docs, configs, and snapshots. Agents load context at session start. `gctl context compact` produces a single LLM-ready document. Sync to R2 makes context available across devices.
...
|-------------|-------------|--------|
| **gctl-board** | Agent-native project management & kanban | P1 shipped (schemas, services) |
| **Observe & Eval** | Langfuse-grade analytics, scoring, prompt management | Partially shipped (analytics, scoring, alerts) |
| **Context Manager** | Kernel extension for shared agent context | Shipped (CRUD, compact, import, HTTP API) |
| **Capacity Engine** | Throughput measurement, delivery forecasting | Planned |
| **Research Assistant** | Crawl docs, build knowledge bases, semantic search | Planned |
| **Code Review Bot** | Auto-review PRs with trace context | Planned |

## Non-Goals

- **Not a cloud platform.** gctl is local-first. Cloud sync is optional. We don't host dashboards or run agents in the cloud.
- **Not an agent framework.** gctl doesn't build agents — it manages them. It works with any agent that accepts a prompt and exits with a status code.
- **Not a replacement for Linear/GitHub/Notion.** gctl connects to these tools via drivers. It provides the kernel underneath, not a replacement.
...
1. A solo developer can install gctl, run `gctl serve`, and get agent visibility + guardrails in < 5 minutes.
...
*For architecture details, see [specs/architecture/](architecture/). For implementation details, see [specs/implementation/](implementation/). For formal verification, see [specs/formal/](formal/).*
