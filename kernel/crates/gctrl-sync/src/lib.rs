//! gctrl-sync — R2 sync engine for local-first, cloud-optional telemetry sync.
//!
...
impl From<SyncError> for gctrl_core::GctlError {
    fn from(e: SyncError) -> Self {
        gctrl_core::GctlError::Sync(e.to_string())
    }
}
...
mod tests {
    ...
    #[test]
    fn sync_error_converts_to_gctrl_error() {
        let err = SyncError::R2("connection refused".into());
        let gctrl_err: gctrl_core::GctlError = err.into();
        assert!(gctrl_err.to_string().contains("connection refused"));
    }
