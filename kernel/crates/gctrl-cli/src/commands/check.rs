use anyhow::Result;
use gctrl_core::{ExecutionContext, GctlConfig, SessionId};
use gctrl_guardrails::GuardrailEngine;
use gctrl_guardrails::policies::{
...
pub fn run(session_id: &str, db_path: &str) -> Result<()> {
    ...
    for (name, decision) in &results {
        let icon = match decision {
            gctrl_core::PolicyDecision::Allow => "PASS",
            gctrl_core::PolicyDecision::Warn(_) => "WARN",
            gctrl_core::PolicyDecision::Deny(_) => "DENY",
        };
