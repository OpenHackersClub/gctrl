//! `gctl context` commands — manage agent context (docs, configs, snapshots).
...
pub fn list(
    ...
    if entries.is_empty() {
        println!("No context entries. Use `gctl context add <file>` to get started.");
        return Ok(());
    }
...
pub fn import_crawl(domain: &str, db_path: &str) -> Result<()> {
    let mgr = open_manager(db_path)?;
    let site_store = gctl_net::SiteStore::default_store()?;

    let index = site_store
        .load_index(domain)
        .ok_or_else(|| anyhow::anyhow!("crawled domain not found: {}. Use `gctl net crawl` first.", domain))?;
