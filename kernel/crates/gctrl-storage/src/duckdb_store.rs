...
fn row_to_session(row: &duckdb::Row<'_>) -> Result<Session> {
    ...
    Ok(Session {
        ...
        workspace_id: gctrl_core::WorkspaceId(workspace_id),
        device_id: gctrl_core::DeviceId(device_id),
        agent_name,
...
fn row_to_span(row: &duckdb::Row<'_>) -> Result<Span> {
    ...
    Ok(Span {
        span_id: gctrl_core::SpanId(span_id),
        trace_id: gctrl_core::TraceId(trace_id),
        parent_span_id: parent_span_id.map(gctrl_core::SpanId),
        session_id: gctrl_core::SessionId(session_id),
        agent_name,
        operation_name,
        span_type: gctrl_core::SpanType::from_str(&span_type_str),
        model,
...
fn row_to_traffic(row: &duckdb::Row<'_>) -> Result<TrafficRecord> {
    ...
    Ok(TrafficRecord {
        ...
        duration_ms: duration_ms as u64,
        session_id: session_id.map(gctrl_core::SessionId),
    })
}
...
mod tests {
    ...
    fn make_project(id: &str, key: &str) -> gctrl_core::BoardProject {
        gctrl_core::BoardProject {
            id: id.into(),
    ...
    fn make_issue(id: &str, project_id: &str) -> gctrl_core::BoardIssue {
        gctrl_core::BoardIssue {
            id: id.into(),
    ...
    fn test_board_issue_crud() {
        ...
        assert_eq!(fetched.status, gctrl_core::IssueStatus::Backlog);
