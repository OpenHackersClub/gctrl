...
/-- Severity is bounded: Allow ≤ d ≤ Deny for all decisions. -/
theorem severity_bounded : ∀ d : PolicyDecision,
    PolicyDecision.allow.severity ≤ d.severity ∧
    d.severity ≤ PolicyDecision.deny.severity := by
  intro d; cases d <;> simp [PolicyDecision.severity]

/-- Severity is a total order. -/
...
/-- The embedding is injective and excludes System. -/
theorem actorKind_embedding : (∀ (a b : ActorKind), a.toUserKind = b.toUserKind → a = b) ∧
    (∀ a : ActorKind, a.toUserKind ≠ .system) := by
  constructor
